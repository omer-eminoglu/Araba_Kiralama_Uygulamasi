...
try {
    ...
    $sql = "INSERT INTO rezervasyon (arac_id, arac_detay_id, musteri_id, teslim_alinacak_tarih, teslim_edilecek_tarih, teslim_alinacak_saat, teslim_edilecek_saat, teslim_alinacak_sube_id, teslim_edilecek_sube_id, odeme_id, ucus_kodu) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($baglan, $sql);
    mysqli_stmt_bind_param($stmt, "iiissssiiis", $aracId, $aracDetayId, $musteriId, $pickupDate, $returnDate, $pickupTime, $returnTime, $pickupSubeId, $returnSubeId, $odemeId, $flightCode);
    mysqli_stmt_execute($stmt);
